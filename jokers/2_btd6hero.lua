...
SMODS.Joker{ --Silas (v50)
    ...
    calculate = function(self, card, context)
            ...
            return {
                func = function()
                    ...
                    if #G.jokers.cards + G.GAME.joker_buffer < G.jokers.config.card_limit then
                        ...
                        G.E_MANAGER:add_event(Event({
                            func = function()
                                local joker_card = SMODS.add_card({ set = 'Joker', key = 'j_pud_iciclez' })
                                if joker_card then
        ...
        if context.other_joker  then
            if (context.other_joker.config.center.key == "j_pud_iciclez" or context.other_joker.config.center.key == "j_pud_iciclezalt") then
                return {
