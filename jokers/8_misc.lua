...
function Card:get_id()
    ...
    if next(SMODS.find_card("j_pud_doreostream")) then
        return 7
    end
    return original_id
end
...
SMODS.Joker{ --Iciclez_
    ...
    calculate = function(self, card, context)
        if context.setting_blind or context.forcetrigger then
            ...
            G.E_MANAGER:add_event(Event({
                func = function()
                    local joker_card = SMODS.add_card({ set = 'Joker', key = 'j_pud_iciclezalt' })
                    if joker_card then
...
G.FUNCS.check_for_buy_space = function(card)
    if card.config.center.key == "j_pud_iciclezalt" then -- ignore slot limit when bought
        return true
    end
...
G.FUNCS.can_select_card = function(e)
	if e.config.ref_table.config.center.key == "j_pud_iciclezalt" then
