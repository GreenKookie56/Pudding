...
SMODS.Joker{ --Carrier Flagship
    ...
    loc_vars = function(self, info_queue, card)
        local new_numerator, new_denominator = SMODS.get_probability_vars(card, 1, card.ability.extra.odds, 'j_pud_carrierflagship') 
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.play then
            if context.other_card:is_suit("Clubs") then
                if SMODS.pseudorandom_probability(card, 'group_0_86561525', 1, card.ability.extra.odds, 'j_pud_carrierflagship', false) then
                    return {
    ...
    calculate = function(self, card, context)
        if context.ending_shop or context.forcetrigger then
            ...
            G.E_MANAGER:add_event(Event({
                func = function()
                    local joker_card = SMODS.add_card({ set = 'Joker', key = 'j_pud_bossfarmingguide' })
                    if joker_card then
