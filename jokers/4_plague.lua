    ...
    calculate = function(self, card, context)
        ...
        if context.setting_blind and not context.blueprint then
                ...
                for i, joker in ipairs(G.jokers.cards) do
                    if joker.config.center.key == "j_pud_bacteria" then
                        target_joker = joker
                        break
                    end
                end
...
function Card:set_cost()
    ...
    if next(SMODS.find_card("j_pud_nanovirus")) then
...
SMODS.Joker{ --Prion
    ...
    loc_vars = function(self, info_queue, card)
        local new_numerator, new_denominator = SMODS.get_probability_vars(card, 1, card.ability.extra.odds, 'j_pud_prion') 
    ...
    calculate = function(self, card, context)
        if context.end_of_round and context.main_eval and G.GAME.blind.boss  then
            if true then
                if SMODS.pseudorandom_probability(card, 'group_0_ebc71fee', 1, card.ability.extra.odds, 'j_pud_prion', false) then
...
SMODS.Joker{ --Neurax worm
    ...
    loc_vars = function(self, info_queue, card)
        local new_numerator, new_denominator = SMODS.get_probability_vars(card, 1, card.ability.extra.odds, 'j_pud_neuraxworm') 
    ...
    calculate = function(self, card, context)
        if context.individual and context.cardarea == G.hand and not context.end_of_round then
            if true then
                if SMODS.pseudorandom_probability(card, 'group_0_ea22710b', 1, card.ability.extra.odds, 'j_pud_neuraxworm', false) then
					return {
